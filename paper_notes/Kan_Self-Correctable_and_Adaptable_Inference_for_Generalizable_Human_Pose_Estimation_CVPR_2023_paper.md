...
![Self-Correction Framework](images/Kan_Self-Correctable_and_Adaptable_Inference_for_Generalizable_Human_Pose_Estimation_CVPR_2023_paper_fig_1.png)

- 📌 **自己参照フィードバック（Self-referential feedback）**: 予測した姿勢を元の画像に逆マッピングして矛盾を計算。このエラー値が、驚くことに実際の予測誤差と**強く相関（-0.84）**することを発見！
- 📌 **テスト時の動的な自己修正**: 正解が分からない推論時（テスト時）でも、この自己参照エラーを頼りに、オクルージョン等でズレた関節を動的に補正し、精度を大幅に向上させました。
...
> [!TIP]
> **🎯 「Ground Truthなし」でのTrue Outlierの画期的な検知手法！**

### 1. 逆マッピングによる「真の外れ値」検知
田中様の研究における最大の難関「何が本当のOutlierなのか？」に対する答えになり得ます。MediaPipeが推定した座標周辺の画像特徴と、過去のフレームの特徴を比較する小さな評価モジュール（FFN）を追加すれば、正解データなしにOutlierを正確に弾けます！

### 2. 遠位関節（手首・足首）に特化したエラー補正
エラーの大部分は可動域が広く隠れやすいDistal keypoints（手首・足首）で発生します。肩や腰などのProximal（近位）関節を「基準アンカー」として固定し、不安定な手首・足首だけを別の軽量ネットワークで再計算するアーキテクチャが非常に有効です。
